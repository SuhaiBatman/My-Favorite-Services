...
export async function fetchConfirmedContact(
  user: User | null,
): Promise<ConfirmedContact> {
...
export async function verifyPasswordReauth(
  ...
  if (error) {
    throw new Error(
      "Incorrect password. Your email and phone were not changed.",
    );
  }
}
...
export async function verifyReauthenticationOtp(
  ...
  if (error) {
    throw new Error(
      "Verification failed. Your email and phone were not changed.",
    );
  }
}
...
export async function confirmPhoneChange(
  ...
  if (error) {
    ...
    throw new Error(
      "Phone verification failed. Your phone number was not changed.",
    );
  }
...
export async function cancelPendingPhoneChange(
  confirmedAuthPhoneE164: string | null,
): Promise<void> {
  if (confirmedAuthPhoneE164) {
    const { error } = await supabase.auth.updateUser({
      phone: confirmedAuthPhoneE164,
    });
    if (error) {
